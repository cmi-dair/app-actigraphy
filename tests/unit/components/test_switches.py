...
def test_toggle_exclude_night(
    mocker: plugin.MockerFixture,
    file_manager: dict[str, str],
) -> None:
    """Test the toggle_exclude_night function."""
    mocker.patch(
        "actigraphy.components.switches._toggle_bool_field",
        return_value=None,
    )
    mocker.patch(
        "actigraphy.components.switches.ggir_files.write_vector",
    )
    func = callback_test_manager.get_callback("toggle_exclude_night")

    actual = func(True, 0, file_manager)

    assert actual is None


def test_toggle_review_night(
    mocker: plugin.MockerFixture,
    file_manager: dict[str, str],
) -> None:
...
def test_toggle_nap(mocker: plugin.MockerFixture, file_manager: dict[str, str]) -> None:
